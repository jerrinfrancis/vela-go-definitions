...
var _ = Describe("ReadOnly Policy", func() {
	...
	Describe("Metadata", func() {
		It("should have the correct name", func() {
			Expect(policy.GetName()).To(Equal("read-only"))
		})
	...
	Describe("CUE Generation", func() {
		...
		Describe("Policy header", func() {
			It("should quote the hyphenated name", func() {
				Expect(cueOutput).To(ContainSubstring(`"read-only": {`))
			})
		...
		Describe("Parameter block", func() {
			...
			It("should include usage comment for rules", func() {
				Expect(cueOutput).To(ContainSubstring("// +usage=Specify the list of rules to control read only strategy"))
			})
		})
		...
		Describe("Structural ordering", func() {
			...
			It("should have header before template", func() {
				headerIdx := strings.Index(cueOutput, `"read-only": {`)
	...
	Describe("YAML Generation", func() {
		It("should produce valid YAML with correct structure", func() {
			...
			Expect(yamlStr).To(ContainSubstring("name: read-only"))
		})
	})
})
